...
final class CountryPropertySpec extends AnyWordSpec with Matchers with ScalaCheckPropertyChecks {
  ...
  private val subdivisionGen: Gen[Subdivision] = for {
    country     <- Gen.oneOf(Country.values.filter(_.subdivisions.nonEmpty))
    subdivision <- Gen.oneOf(country.subdivisions)
  } yield subdivision
