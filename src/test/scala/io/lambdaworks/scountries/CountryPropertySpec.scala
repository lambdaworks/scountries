...
final class CountryPropertySpec extends AnyWordSpec with Matchers with ScalaCheckPropertyChecks {
  ...
  implicit val arbitraryCountry: Arbitrary[Country] = Arbitrary {
    Gen.oneOf(Country.values)
  }

  implicit val arbitrarySubdivision: Arbitrary[Subdivision] = Arbitrary {
    Gen.oneOf(Country.values.flatMap(_.subdivisions))
  }
